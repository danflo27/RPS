...
func TestQueryCounters(t *testing.T) {
	...
	resp, err := f.queryServer.Counters(f.ctx, &rps.QueryCountersRequest{})
	require.NoError(err)
...
func TestQueryCountersPaginated(t *testing.T) {
	...
	resp, err := f.queryServer.Counters(f.ctx, &rps.QueryCountersRequest{Pagination: &query.PageRequest{Limit: 1}})
	require.NoError(err)
	require.Equal(0, len(resp.Counters))

	_, err = f.msgServer.IncrementCounter(f.ctx, &rps.MsgIncrementCounter{Sender: f.addrs[0].String()})
	require.NoError(err)
	_, err = f.msgServer.IncrementCounter(f.ctx, &rps.MsgIncrementCounter{Sender: f.addrs[1].String()})
	require.NoError(err)
